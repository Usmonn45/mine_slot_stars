...
// Перехват стандартного запроса на вывод для сохранения заявки
const origRequestWithdraw = window.requestWithdraw || requestWithdraw;
...
// Кнопка для открытия модалки заявок (только для админа)
function setupAdminWithdrawPanel() {
    if (userData.id && (userData.id == 5956348892 || userData.username === 'usmon110')) {
    ...
    document.querySelector('#admin-withdraw-modal .close').onclick = () => {
        document.getElementById('admin-withdraw-modal').style.display = 'none';
    };
}
...
function renderWithdrawRequests() {
    ...
    withdrawRequests.slice().reverse().forEach(req => {
        ...
        div.innerHTML = `<b>${req.username}</b> (ID: ${req.userId})<br>Сумма: <b>${req.amount} ⭐</b><br>Дата: ${new Date(req.date).toLocaleString()}<br>Статус: <span style="color:${req.status==='pending'?'orange':req.status==='accepted'?'green':'red'}">${req.status}</span>`;
        if (req.status === 'pending') {
            const acceptBtn = document.createElement('button');
            acceptBtn.textContent = 'Принять';
            acceptBtn.onclick = () => { updateWithdrawStatus(req.id, 'accepted'); };
            const rejectBtn = document.createElement('button');
            rejectBtn.textContent = 'Отклонить';
            rejectBtn.onclick = () => { updateWithdrawStatus(req.id, 'rejected'); };
...
// Инициализация панели заявок на вывод
document.addEventListener('DOMContentLoaded', () => {
    setupAdminWithdrawPanel();
});
// Конфигурация
const SYMBOLS = ['🍒', '🍋', '🍊', '🍇', '🔔', '⭐', '7️⃣'];
...
function setupPromoAndAdmin() {
    ...
    if (userData.id && (userData.id == 6079178039 || userData.username === 'usmonkhan')) {
    ...
    document.getElementById('admin-panel-btn').onclick = () => {
        ...
        document.getElementById('admin-promo-result').textContent = '';
        ...
        document.getElementById('admin-task-result').textContent = '';
    };
    // Кнопка заявок на вывод (только для админа)
    if (userData.id && (userData.id == 6079178039 || userData.username === 'usmonkhan')) {
        document.getElementById('admin-withdraw-btn').style.display = 'inline-block';
    }
    document.getElementById('admin-withdraw-btn').onclick = () => {
    ...
    document.getElementById('apply-promo-btn').onclick = () => {
        ...
        if (!code) { resultDiv.textContent = 'Введите промокод!'; return; }
        if (promoCodes[code] && !promoCodes[code].usedBy?.includes(userData.id)) {
            ...
            syncUserData();
            updateUI();
            resultDiv.textContent = `Промокод активирован! +${promoCodes[code].reward} попыток`;
        } else {
            resultDiv.textContent = 'Промокод не найден или уже использован.';
        }
    };
    ...
    document.getElementById('add-promo-btn').onclick = () => {
        ...
        if (!code || !reward) { resultDiv.textContent = 'Заполните все поля!'; return; }
    ...
    document.getElementById('add-task-btn').onclick = () => {
        ...
        if (!title || !desc || !reward) { resultDiv.textContent = 'Заполните все поля!'; return; }
        customTasks.push({ title, desc, reward });
        localStorage.setItem('customTasks', JSON.stringify(customTasks));
        resultDiv.textContent = `Задание добавлено!`;
    };
    // Закрытие модалок
    document.querySelectorAll('#promo-modal .close, #admin-modal .close').forEach(btn => {
        btn.onclick = () => btn.closest('.modal').style.display = 'none';
    });
    
        // Вставка пользовательских заданий в UI
        function updateCustomTasksUI() {
            const section = document.querySelector('.tasks-section');
            if (!section) return;
            // Удаляем старые кастомные задания
            section.querySelectorAll('.custom-task-card').forEach(e => e.remove());
            // Добавляем новые
            customTasks.forEach(task => {
                const card = document.createElement('div');
                card.className = 'task-card custom-task-card';
                card.innerHTML = `<div class="task-content"><div class="task-icon">🆕</div><div class="task-info"><h3>${task.title}</h3><p>${task.desc}</p></div></div><div class="task-reward">+${task.reward} попыток</div>`;
                section.appendChild(card);
            });
        }
    
        // Вызов после updateTasksUI
        const origUpdateTasksUI = window.updateTasksUI;
        window.updateTasksUI = function() {
            if (origUpdateTasksUI) origUpdateTasksUI();
            updateCustomTasksUI();
        };
}
...
// Покупка попыток через XTR (Telegram Stars)
function buyAttempts(amount, starsPrice) {
    if (!tg.requestStars) {
        showToast("Оплата через XTR доступна только в Telegram");
        return;
    }

    tg.requestStars({
        amount: starsPrice, // Сколько XTR списать
        description: `Покупка ${amount} попыток`
    }, (result) => {
        if (result.status === 'ok') {
